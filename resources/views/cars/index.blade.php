...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">Список автомобилей</h1>
        @if ($cars->isEmpty())
                ...
                @foreach ($cars as $car)
                    <tr class="@if($loop->odd) bg-gray-300 @endif">
                        ...
                        <td class="p-2 text-center">
                            <a href="{{route('cars.show', $car->id)}}" class="underline underline-offset-2">
                                {{$car->client->name}}
                            </a>
                        </td>
                        <td></td>
                        <td class="p-2 flex flex-col text-center items-center">
                            <a href="{{ route('clients.edit', $car->id) }}" class="flex justify-center p-1 bg-blue-400 rounded-md">
                        ...
                        <td></td>
                        <td class="p-2 flex flex-col text-center items-center">
                            <form action="{{ route('clients.destroy', $car->id) }}" class="flex justify-center" method="POST">
                                @csrf
                                @method('DELETE')
